...
export class Experience {
  ...
  private constructor(private readonly container: HTMLElement, quality: Quality) {
    // WebGPURenderer cai sozinho pra WebGL2 quando o navegador não tem WebGPU
    this.renderer = new THREE.WebGPURenderer({ antialias: false, powerPreference: 'high-performance' })
    this.ctx = { pointer: new THREE.Vector2(0.5, 0.5), width: 1, height: 1, quality }
  }
  ...
  private observeVisibility() {
    const observer = new IntersectionObserver(([entry]) => { this.visible = entry.isIntersecting })
    observer.observe(this.container)
    this.cleanups.push(() => observer.disconnect())
  }
