...
import {
  Fn, uniform, uv, time, vec2, vec3, vec4, color,
  mix, smoothstep, distance, hash, screenUV, mx_noise_float,
} from 'three/tsl'
import type { Feature, FrameContext } from '../types'

/**
 * Fase 1: gradiente vivo em TSL na paleta do site (#7947DF).
...
export class HeroBackground implements Feature {
  ...
  private readonly mesh: THREE.Mesh<THREE.PlaneGeometry, THREE.MeshBasicNodeMaterial>
  ...
  private buildColorNode() {
    ...
    return Fn(() => {
      ...
      const n = mx_noise_float(vec3(st.mul(2.0).add(warp.mul(0.9)), t.mul(1.5)))

      const deep = color(0x0e0a1a)
      const mid = color(0x2a1760)
      const brand = color(0x7947df)
      ...
      const glow = smoothstep(0.0, 0.6, distance(st, pointer)).oneMinus().mul(0.12)

      // dithering: evita banding em gradientes escuros (fica feio em monitor 8-bit)
      const grain = hash(screenUV.mul(4096)).sub(0.5).mul(1 / 255)
